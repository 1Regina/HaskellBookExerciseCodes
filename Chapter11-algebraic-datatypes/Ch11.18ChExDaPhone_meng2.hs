...
reverseTaps d'phone wantc =
  ...
  where
    ...
    lowers = [ (char, [ (digit, presses) ] )
             | (digit, abc)     <- d'phone
             , (char,  presses) <- zip abc [1..] -- for each (digit, abc) in d'phone: for each (char,  presses)  in zip abc [1..]: return (char, [ (digit, presses) ] )
             ]
...
fingerTaps = sum . (fmap snd) -- sum of the second tuple ie how many times the finger taps on the dialtaps
